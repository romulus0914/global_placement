...
ExampleFunction::ExampleFunction(Placement &placement) : _placement(placement)
{
    ...
    Tb = CalculateTargetBinDensity(width, height);
...
double ExampleFunction::CalculateTargetBinDensity(const double width, const double height)
{
...
void ExampleFunction::InitializeBins(const double width, const double height)
{
    ...
    for (unsigned int y_idx = 0; y_idx < num_bins_per_row; y_idx++) {
        double y_center = boundary_bottom + (0.5 + y_idx) * bin_height;
        for (unsigned int x_idx = 0; x_idx < num_bins_per_row; x_idx++) {
            double x_center = boundary_left + (0.5 + x_idx) * bin_width;
            bins[y_idx][x_idx].x_center = x_center;
            bins[y_idx][x_idx].y_center = y_center;
        }
    }
}
    ...
    for (unsigned int y_idx = 0; y_idx < num_bins_per_row; y_idx++) {
        for (unsigned int x_idx = 0; x_idx < num_bins_per_row; x_idx++) {
            ...
            for (unsigned int module_id = 0; module_id < num_modules; module_id++) {
                ...
                double dx = abs(x[module_id * 2] - bins[y_idx][x_idx].x_center);
                ...
                double dy = abs(x[module_id * 2 + 1] - bins[y_idx][x_idx].y_center);
            ...
            total_bin_density += density_diff * density_diff;
        }
    }
    ...
    for (unsigned int y_idx = 0; y_idx < num_bins_per_row; y_idx++) {
        for (unsigned int x_idx = 0; x_idx < num_bins_per_row; x_idx++) {
            ...
            for (unsigned int module_id = 0; module_id < num_modules; module_id++) {
                ...
                double dx = x[module_id * 2] - bins[y_idx][x_idx].x_center;
                ...
                double dy = x[module_id * 2 + 1] - bins[y_idx][x_idx].y_center;
            ...
            for (unsigned int module_id = 0; module_id < num_modules; module_id++) {
                //g[module_id * 2] += beta * 2 * (bin_density - Tb) * g_temp[module_id * 2];
                //g[module_id * 2 + 1] += beta * 2 * (bin_density - Tb) * g_temp[module_id * 2 + 1];
                g[module_id * 2] += beta * 2 * density_diff * g_temp[module_id * 2];
                g[module_id * 2 + 1] += beta * 2 * density_diff * g_temp[module_id * 2 + 1];
            }
        }
    }
